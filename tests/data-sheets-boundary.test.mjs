...
test('Sheets export includes stable identity and append filters previously successful revisions', async()=>{
  ...
  let out=await c.buildSheetsExport('p1',{mode:'append'}); assert.equal(out.entries.length,1); assert.equal(out.entries[0].clipkit_entry_id,'e1'); assert.equal(out.entries[0].Full_Key,'Media');
